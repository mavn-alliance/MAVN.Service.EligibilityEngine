...
using Lykke.Service.EligibilityEngine;
using Lykke.Service.EligibilityEngine.Client;
using Microsoft.AspNetCore.Mvc;
using Refit;
using Xunit;

namespace Lykke.Service.EligibilityEngine.Tests
{
    public class ClientHttpPathsAreMatchedInControllersTest
    {
        ...
        {
            var clientInterface = typeof(IEligibilityEngineClient);
            ...
            foreach (var apiInterface in apiInterfaces)
            {
                ...
                if (implementingController == null)
                {
                    if (interfaceMethods.Length > 0)
                        apiErrors.Add($"ConversionRateResponse interface '{apiInterface.Name}' is not implemented");
                    continue;
                }
